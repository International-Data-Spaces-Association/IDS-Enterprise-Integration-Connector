...
import de.fraunhofer.iais.eis.ids.mdmconnector.components.DefaultSuccessMAP;
...
public class ContractAgreementMessageHandler implements MessageHandler<ContractAgreementMAP, MessageAndPayload<Message, Object>> {
	 ...
	 */
	public ContractAgreementMessageHandler(InfrastructureComponent infrastructureComponent, DapsSecurityTokenProvider daps, ArtifactIndex artifactIndex, String negotiationServiceURL) {
		...
		try {
			...
			logger.info("...passed:");
				if (contractUtil.compareWithIndex(agreementContract, message.getSenderAgent())) {
					MessageProcessedNotificationMessage confirm = new MessageProcessedNotificationMessageBuilder()
